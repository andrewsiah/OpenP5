    ...
    if args.dist_sampler == 0:
        train_sampler = (
            ...
            if args.distributed
                ...
                TrainSet, args.batch_size, args.seed, shuffle=True
            )
        )
    else:
...
def distributed_main(local_rank, args):
    ...
    train_loader, valid_loader = get_loader(
        args, tokenizer, TrainSet, ValidSet, local_rank
    )
    ...
    runner = DistributedRunner(
        model, tokenizer, train_loader, valid_loader, device, args, local_rank
    )
